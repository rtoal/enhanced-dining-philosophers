...
with Philosophers, Cooks, Host, Meals, Reporter, Protected_Counters;
use Philosophers, Cooks, Host, Meals, Reporter, Protected_Counters;
...
package body Waiters is
   ...
   Waiters_Alive : Counter (Waiter_Tasks'Length);
   ...
   task body Waiter is
      ...
      begin
         select
            ...
            Philosopher_Tasks (An_Order.Diner).Here_Is_Your_Food;
         or
            delay 2.0;
         ...
         select
            accept Place_Order (
               ...
               Report (
                  My_Name &
                  " takes an order from " &
                  Philosopher_Name'Image (Customer));
...
begin
   for I in Waiter_Tasks'Range loop
      Waiter_Tasks (I) := new Waiter;
   end loop;
end Waiters;
