...
--  edp.adb
--
--  A compact simulation of the Enhanced Dining Philosophers Problem.
--
...
--    2 waiters;
--    3 cooks;
--    A host, who lets philosophers into the restaurant and escorts them out;
...
with Philosophers, Waiters, Cooks, Reporter;
use Philosophers, Waiters, Cooks, Reporter;
...
begin
   ...
   for W in Waiter_Tasks'Range loop
      Waiter_Tasks (W).Here_Is_Your_Name (W);
   end loop;
   for P in Philosopher_Tasks'Range loop
      Philosopher_Tasks (P).Here_Is_Your_Name (P);
   end loop;
end EDP;
