...
--   uses a single private random number generator for floats; this generator
--   is called to obtain both integers and floats by using appropriate scaling.
------------------------------------------------------------------------------
...
package body Randoms is
   ...
   --  X .. Y is done by computing X+R*(Y-X), however the computation Y-X may
   --  overflow for very large Y and very small X. So we distribute the
   --  multiplication, and order terms in such a way as to
   --  avoid overflow.

   function Random (X, Y : Float) return Float is
      R : constant Float := Random (G);
   begin
      return X - X * R + Y * R;
   end Random;
   ...
   function Random_Discrete return Element is
      Min : constant Integer := Element'Pos (Element'First);
      Max : constant Integer := Element'Pos (Element'Last);
   begin
